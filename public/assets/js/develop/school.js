function loadStudentMarksTable(standardSelectId, groupSelectId, sectionSelectId, subjectSelectId, tableContainerId, base_url) {
    $('#' + standardSelectId + ', #' + groupSelectId + ', #' + sectionSelectId + ', #' + subjectSelectId).on('change', function(){
        var standard = $('#' + standardSelectId).val();
        var group = $('#' + groupSelectId).val() || '';
        var section = $('#' + sectionSelectId).val() || '';
        var subject = $('#' + subjectSelectId + ' option:selected').text();

        var $tableContainer = $('#' + tableContainerId);
        $tableContainer.empty();

        if(standard && section && subject){
            $.ajax({
                url: base_url + '/get-students/' + standard + (group ? '/' + group : '') + '/' + section,
                type: 'GET',
                success: function(students){
                    if(students.length > 0){
                        let table = `<table class="table table-bordered" id="marks-table">
                                        <thead>
                                            <tr>
                                                <th>S.No</th>
                                                <th>Enroll.NO</th>
                        ...
                        $.each(students, function(index, student){
                            table += `<tr>
                                ...
                                <td>${student.name}</td>
                                <td>
                                    <input type="number" class="form-control marks-input" data-index="${index}" value="0" min="0" />
                                </td>
                                <td class="total" data-index="${index}">0</td>
                            </tr>`;
                        });
                        table += `</tbody></table>`;
                        ...
                        $('.marks-input').on('input', function(){
                            ...
                            $('.total[data-index="'+idx+'"]').text(val); // you can expand to multiple subjects later
                        });
                    } else {
                        $tableContainer.append('<p>No students found for this selection.</p>');
                    }
                }
            });
        }
